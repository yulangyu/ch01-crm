...
<script type="text/javascript">
	...
	$(function(){
		...
		//为关联市场活动模态窗口中的搜索框绑定事件,通过触发回车键触发查询所需市场活动列表
		$("#aname").keydown(function (event) {
			//如果是回车键
			if (event.keyCode==13){
            //展现完列表后，记得将模态窗口默认的回车行为禁用掉
				$.ajax({
					url:"workbench/clue/getActivityByIdAndNotByClueId.do",
					data:{
				...
				$.ajax({
					...
					success:function (data){
						if (data.success){
                            ...
                            //刷新
							showActivityList();
							//清楚搜索框中的信息  复选框的勾干掉  清空activitySearchBody中的内容
		...
		$.ajax({
			...
			success:function (data){
				if (data.success){
					...
					$("#"+id).remove();
					//也可以重新刷新页面，因为<tr>标签为HTML标签，showActivityList();
				}else{
					alert("删除失败");
				}
			}
...
<body>
	...
	<div style="position: relative; left: 40px; top: -30px;">
		...
		<div style="position: relative; height: 50px; width: 500px;  top: -72px; left: 700px;">
			<button type="button" class="btn btn-default" onclick="window.location.href='workbench/clue/convert.jsp?fullname=${c.fullname}&id=${c.id}&appellation=${c.appellation}&company=${c.company}&owner=${c.owner}';"><span class="glyphicon glyphicon-retweet"></span> 转换</button>
